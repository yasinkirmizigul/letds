    ...
    <div class="mx-auto max-w-7xl px-4 py-8">
                    ...
                    <div class="{{ $loop->first ? '' : 'hidden' }} relative min-h-[520px]" data-home-slide="true">
                        ...
                        <div class="relative z-10 grid min-h-[520px] gap-10 px-6 py-10 lg:grid-cols-[minmax(0,1fr)_280px] lg:px-12 lg:py-14">
                            <div class="flex flex-col justify-end">
                                ...
                                @endif
                                <h1 class="max-w-3xl text-4xl font-semibold leading-tight md:text-5xl">{{ $slider->title }}</h1>
                                @if($slider->subtitle)
                                    <p class="mt-5 max-w-2xl text-base leading-8 text-white/80 md:text-lg">{{ $slider->subtitle }}</p>
                                @endif
                                @if($slider->body)
                                    <div class="mt-5 max-w-2xl text-sm leading-7 text-white/70">{{ $slider->body }}</div>
                                @endif
                                @if($slider->cta_label && $slider->cta_url)
                                    <div class="mt-8">
                                        <a href="{{ $slider->cta_url }}" class="kt-btn kt-btn-primary">{{ $slider->cta_label }}</a>
                                    </div>
                                @endif
                            </div>
        ...
        @if($featuredPages->isNotEmpty())
            <section class="mt-14">
                <div class="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                    <div>
                        <div class="text-xs font-semibold uppercase tracking-[0.28em] text-primary">Öne Çıkan Sayfalar</div>
                        <h2 class="mt-2 text-3xl font-semibold text-foreground">Yönettiğin içerikler doğrudan sahnede</h2>
                    </div>
                </div>

                <div class="grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
                    @foreach($featuredPages as $page)
                        <a href="{{ $page->publicUrl() }}" class="group rounded-[32px] border border-border bg-white/85 p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                            <div class="flex items-start justify-between gap-4">
                                <div class="inline-flex size-12 items-center justify-center rounded-2xl bg-primary/10 text-primary">
                                    <i class="{{ $page->icon_class ?: 'ki-filled ki-abstract-26' }}"></i>
                                </div>
                                <span class="rounded-full bg-muted/70 px-3 py-1 text-xs text-muted-foreground">/{{ $page->slug }}</span>
                            </div>
                            <h3 class="mt-5 text-xl font-semibold text-foreground">{{ $page->title }}</h3>
                            <p class="mt-3 text-sm leading-7 text-muted-foreground">{{ $page->excerptPreview(150) }}</p>
                            <div class="mt-5 text-sm font-medium text-primary">Sayfayı Aç</div>
                        </a>
                    @endforeach
                </div>
            </section>
        ...
        @if($globalFaqs->isNotEmpty())
            <section class="mt-14 grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
                <div>
                    ...
                    <h2 class="mt-2 text-3xl font-semibold text-foreground">Ziyaretçinin kararını hızlandıran net cevaplar</h2>
                    <p class="mt-4 text-sm leading-7 text-muted-foreground">
                        Yönetim panelinden eklediğin global SSS kayıtları burada otomatik olarak görünür.
                    </p>
                </div>
        ...
        <section class="mt-14 grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
            <div class="rounded-[32px] border border-border bg-white/85 p-6 shadow-sm">
                <div class="text-xs font-semibold uppercase tracking-[0.28em] text-primary">İletişim</div>
                <h2 class="mt-2 text-3xl font-semibold text-foreground">Bir sonraki adımı birlikte planlayalım</h2>
                <div class="mt-4 grid gap-3 text-sm leading-7 text-muted-foreground">
                    ...
                    @if($siteSettings->address_line)
                        <div>Adres: {{ $siteSettings->address_line }}</div>
                    @endif
                    @if($siteSettings->office_hours)
                        <div>Mesai: {{ $siteSettings->office_hours }}</div>
                    @endif
                </div>
                <div class="mt-6 flex flex-wrap gap-3">
                    <a href="{{ route('site.contact-messages.create') }}" class="kt-btn kt-btn-primary">Mesaj Gönder</a>
                    <a href="{{ auth('member')->check() ? route('member.appointments.index') : route('member.login') }}" class="kt-btn kt-btn-light">
            ...
            @if($siteSettings->map_embed_url)
                <div class="overflow-hidden rounded-[32px] border border-border bg-white shadow-sm">
                    <iframe
                        ...
                        title="{{ $siteSettings->map_title ?: 'Harita' }}"
