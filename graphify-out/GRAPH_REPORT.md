# Graph Report - letds  (2026-08-28)

## Corpus Check
- 645 files · ~251,799 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3839 nodes · 8353 edges · 430 communities (276 shown, 154 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 695 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0affa3b9`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_IlluminateHttpRedirectResponse|Illuminate\Http\RedirectResponse]]
- [[_COMMUNITY_BlogPost|BlogPost]]
- [[_COMMUNITY_history-timeline.js|history-timeline.js]]
- [[_COMMUNITY_calendar.js|calendar.js]]
- [[_COMMUNITY_AuditEvent|AuditEvent]]
- [[_COMMUNITY_EcommerceOrder|EcommerceOrder]]
- [[_COMMUNITY_Role|Role]]
- [[_COMMUNITY_Project|Project]]
- [[_COMMUNITY_ModuleNamer|ModuleNamer]]
- [[_COMMUNITY_IlluminateDatabaseEloquentModel|Illuminate\Database\Eloquent\Model]]
- [[_COMMUNITY_PaymentIntegration|PaymentIntegration]]
- [[_COMMUNITY_dependencies|dependencies]]
- [[_COMMUNITY_Member|Member]]
- [[_COMMUNITY_.now|.now]]
- [[_COMMUNITY_adminpagesindex.js|admin/pages/index.js]]
- [[_COMMUNITY_Product|Product]]
- [[_COMMUNITY_AdminMenuRegistry|AdminMenuRegistry]]
- [[_COMMUNITY_settings.js|settings.js]]
- [[_COMMUNITY_LocalizedContentTranslationService|LocalizedContentTranslationService]]
- [[_COMMUNITY_.index|.index]]
- [[_COMMUNITY_blogform-shared.js|blog/form-shared.js]]
- [[_COMMUNITY_ContactMessage|ContactMessage]]
- [[_COMMUNITY_Closure|Closure]]
- [[_COMMUNITY_Appointment|Appointment]]
- [[_COMMUNITY_productsform-shared.js|products/form-shared.js]]
- [[_COMMUNITY_projectsform-shared.js|projects/form-shared.js]]
- [[_COMMUNITY_AppointmentCalendarController|AppointmentCalendarController]]
- [[_COMMUNITY_EcommerceInvoice|EcommerceInvoice]]
- [[_COMMUNITY_appointmentsindex.js|appointments/index.js]]
- [[_COMMUNITY_ServiceReviewQuestion|ServiceReviewQuestion]]
- [[_COMMUNITY_IlluminateBusQueueable|Illuminate\Bus\Queueable]]
- [[_COMMUNITY_Category|Category]]
- [[_COMMUNITY_AdminQuickSearchController|AdminQuickSearchController]]
- [[_COMMUNITY_User|User]]
- [[_COMMUNITY_0001_01_01_000001_create_cache_table.php|0001_01_01_000001_create_cache_table.php]]
- [[_COMMUNITY_HomepageConfigurationService|HomepageConfigurationService]]
- [[_COMMUNITY_SeoFileGenerator|SeoFileGenerator]]
- [[_COMMUNITY_projectsindex.js|projects/index.js]]
- [[_COMMUNITY_Media|Media]]
- [[_COMMUNITY_HomeSlider|HomeSlider]]
- [[_COMMUNITY_IlluminateDatabaseEloquentBuilder|Illuminate\Database\Eloquent\Builder]]
- [[_COMMUNITY_productsindex.js|products/index.js]]
- [[_COMMUNITY_ProfileController|ProfileController]]
- [[_COMMUNITY_SitePage|SitePage]]
- [[_COMMUNITY_IlluminateFoundationHttpFormRequest|Illuminate\Foundation\Http\FormRequest]]
- [[_COMMUNITY_AdminNotification|AdminNotification]]
- [[_COMMUNITY_ajax-forms.js|ajax-forms.js]]
- [[_COMMUNITY_SiteSetting|SiteSetting]]
- [[_COMMUNITY_up|up]]
- [[_COMMUNITY_TestCase|TestCase]]
- [[_COMMUNITY_EcommerceCoupon|EcommerceCoupon]]
- [[_COMMUNITY_AppointmentSettingsController|AppointmentSettingsController]]
- [[_COMMUNITY_MediaService|MediaService]]
- [[_COMMUNITY_trashindex.js|trash/index.js]]
- [[_COMMUNITY_app-init.js|app-init.js]]
- [[_COMMUNITY_What You Must Do When Invoked|What You Must Do When Invoked]]
- [[_COMMUNITY_SiteSettingsController.php|SiteSettingsController.php]]
- [[_COMMUNITY_blogindex.js|blog/index.js]]
- [[_COMMUNITY_create-form-accordion.js|create-form-accordion.js]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_ordersform.js|orders/form.js]]
- [[_COMMUNITY_member-register.js|member-register.js]]
- [[_COMMUNITY_http.js|http.js]]
- [[_COMMUNITY_SiteDefaultLocalePromotionService|SiteDefaultLocalePromotionService]]
- [[_COMMUNITY_Theme Tailwind HTML Laravel Integration|Theme Tailwind HTML Laravel Integration]]
- [[_COMMUNITY_featured-image-manager.js|featured-image-manager.js]]
- [[_COMMUNITY_SiteFaq|SiteFaq]]
- [[_COMMUNITY_AdminResetPasswordNotification.php|AdminResetPasswordNotification.php]]
- [[_COMMUNITY_adminapp.js|admin/app.js]]
- [[_COMMUNITY_composer.json|composer.json]]
- [[_COMMUNITY_date-input.js|date-input.js]]
- [[_COMMUNITY_datatable-helper.js|datatable-helper.js]]
- [[_COMMUNITY_categoriesindex.js|categories/index.js]]
- [[_COMMUNITY_gallery-manager.js|gallery-manager.js]]
- [[_COMMUNITY_swal-alert.js|swal-alert.js]]
- [[_COMMUNITY_adminlayoutsmainapp.blade.php|admin/layouts/main/app.blade.php]]
- [[_COMMUNITY_Plan 001 Site tasarım temelini kur — fontlar, OKLCH token katmanı, Tailwind @theme eşlemesi, motion altyapısı|Plan 001: Site tasarım temelini kur — fontlar, OKLCH token katmanı, Tailwind @theme eşlemesi, motion altyapısı]]
- [[_COMMUNITY_IlluminateHttpRequest|Illuminate\Http\Request]]
- [[_COMMUNITY_require-dev|require-dev]]
- [[_COMMUNITY_settingsedit.js|settings/edit.js]]
- [[_COMMUNITY_admin-quick-search.js|admin-quick-search.js]]
- [[_COMMUNITY_metronic-pickers.js|metronic-pickers.js]]
- [[_COMMUNITY_CarbonCarbon|Carbon\Carbon]]
- [[_COMMUNITY_StoreContactMessageRequest|StoreContactMessageRequest]]
- [[_COMMUNITY_Plan 003 Ana sayfayı yeniden tasarla — admin verisi sızıntısını kapat, editoryal bölüm ritmi, crossfade slider, scroll-reveal|Plan 003: Ana sayfayı yeniden tasarla — admin verisi sızıntısını kapat, editoryal bölüm ritmi, crossfade slider, scroll-reveal]]
- [[_COMMUNITY_mediaindex.js|media/index.js]]
- [[_COMMUNITY_rolescreate.js|roles/create.js]]
- [[_COMMUNITY_media-upload-modal.js|media-upload-modal.js]]
- [[_COMMUNITY_OrderController|OrderController]]
- [[_COMMUNITY_0001_01_01_000005_create_permission_role_table.php|0001_01_01_000005_create_permission_role_table.php]]
- [[_COMMUNITY_ContentPageController|ContentPageController]]
- [[_COMMUNITY_Plan 005 İkincil sayfaları sisteme hizala — auth, iletişim, CMS sayfası, hesap; SEO skoru sızıntısını kaldır|Plan 005: İkincil sayfaları sisteme hizala — auth, iletişim, CMS sayfası, hesap; SEO skoru sızıntısını kaldır]]
- [[_COMMUNITY_config|config]]
- [[_COMMUNITY_manage.js|manage.js]]
- [[_COMMUNITY_initSliderForm|initSliderForm]]
- [[_COMMUNITY_MemberDocumentService|MemberDocumentService]]
- [[_COMMUNITY_galleriesedit.js|galleries/edit.js]]
- [[_COMMUNITY_menu-visibilityedit.js|menu-visibility/edit.js]]
- [[_COMMUNITY_psr-4|psr-4]]
- [[_COMMUNITY_require|require]]
- [[_COMMUNITY_load-script-once.js|load-script-once.js]]
- [[_COMMUNITY_blogpartials_form.blade.php|blog/partials/_form.blade.php]]
- [[_COMMUNITY_projectspartials_form.blade.php|projects/partials/_form.blade.php]]
- [[_COMMUNITY_navigationindex.js|navigation/index.js]]
- [[_COMMUNITY_`appointments`|`appointments`]]
- [[_COMMUNITY_post-create-project-cmd|post-create-project-cmd]]
- [[_COMMUNITY_2026_08_01_000002_create_site_homepage_configs_table.php|2026_08_01_000002_create_site_homepage_configs_table.php]]
- [[_COMMUNITY_ServiceReview|ServiceReview]]
- [[_COMMUNITY_slug-manager.js|slug-manager.js]]
- [[_COMMUNITY_base.blade.php|base.blade.php]]
- [[_COMMUNITY_homepageedit.js|homepage/edit.js]]
- [[_COMMUNITY_Sluggable|Sluggable]]
- [[_COMMUNITY_autoload-dev|autoload-dev]]
- [[_COMMUNITY_extra|extra]]
- [[_COMMUNITY_MemberDocumentService|MemberDocumentService]]
- [[_COMMUNITY_0001_01_01_000000_create_users_table.php|0001_01_01_000000_create_users_table.php]]
- [[_COMMUNITY_AdminAppointmentNotificationMail|AdminAppointmentNotificationMail]]
- [[_COMMUNITY_.schema|.schema]]
- [[_COMMUNITY_0001_01_01_000003_create_roles_table.php|0001_01_01_000003_create_roles_table.php]]
- [[_COMMUNITY_0001_01_01_000004_create_permissions_table.php|0001_01_01_000004_create_permissions_table.php]]
- [[_COMMUNITY_homepageedit.blade.php|homepage/edit.blade.php]]
- [[_COMMUNITY_PaymentWebhookEvent|PaymentWebhookEvent]]
- [[_COMMUNITY_2025_12_14_192415_create_blog_posts_table.php|2025_12_14_192415_create_blog_posts_table.php]]
- [[_COMMUNITY_2025_12_16_142024_create_categories_table.php|2025_12_16_142024_create_categories_table.php]]
- [[_COMMUNITY_2025_12_16_142051_create_categorizables_table.php|2025_12_16_142051_create_categorizables_table.php]]
- [[_COMMUNITY_site_homepage.php|site_homepage.php]]
- [[_COMMUNITY_static|static]]
- [[_COMMUNITY_.formPayload|.formPayload]]
- [[_COMMUNITY_2025_12_27_133537_create_gallery_items_table.php|2025_12_27_133537_create_gallery_items_table.php]]
- [[_COMMUNITY_2025_12_27_133604_create_galleryables_table.php|2025_12_27_133604_create_galleryables_table.php]]
- [[_COMMUNITY_IlluminateDatabaseEloquentRelationsMorphToMany|Illuminate\Database\Eloquent\Relations\MorphToMany]]
- [[_COMMUNITY_.show|.show]]
- [[_COMMUNITY_2026_02_15_000002_create_category_product_table.php|2026_02_15_000002_create_category_product_table.php]]
- [[_COMMUNITY_2026_03_27_202618_create_members_table.php|2026_03_27_202618_create_members_table.php]]
- [[_COMMUNITY_SiteLanguage|SiteLanguage]]
- [[_COMMUNITY_ProjectStoreRequest|ProjectStoreRequest]]
- [[_COMMUNITY_ProductGalleryController|ProductGalleryController]]
- [[_COMMUNITY_2026_03_28_120222_create_provider_working_hours_table.php|2026_03_28_120222_create_provider_working_hours_table.php]]
- [[_COMMUNITY_.index|.index]]
- [[_COMMUNITY_AdminMenuRegistryTest|AdminMenuRegistryTest]]
- [[_COMMUNITY_2026_04_22_000005_create_site_pages_table.php|2026_04_22_000005_create_site_pages_table.php]]
- [[_COMMUNITY_SiteTranslationSyncService|SiteTranslationSyncService]]
- [[_COMMUNITY_2026_04_22_000007_create_site_counters_table.php|2026_04_22_000007_create_site_counters_table.php]]
- [[_COMMUNITY_2026_04_22_000008_create_site_navigation_items_table.php|2026_04_22_000008_create_site_navigation_items_table.php]]
- [[_COMMUNITY_2026_04_22_000009_create_site_settings_table.php|2026_04_22_000009_create_site_settings_table.php]]
- [[_COMMUNITY_AdminLoginBrandingTest|AdminLoginBrandingTest]]
- [[_COMMUNITY_FaqController.php|FaqController.php]]
- [[_COMMUNITY_AdminLoginBrandingTest|AdminLoginBrandingTest]]
- [[_COMMUNITY_blog-index.js|blog-index.js]]
- [[_COMMUNITY_.formPayload|.formPayload]]
- [[_COMMUNITY_graphify reference query, path, explain|graphify reference: query, path, explain]]
- [[_COMMUNITY_.boot|.boot]]
- [[_COMMUNITY_AdminDashboardPreference.php|AdminDashboardPreference.php]]
- [[_COMMUNITY_2026_04_30_000003_add_sitemap_xml_content_to_site_settings_table.php|2026_04_30_000003_add_sitemap_xml_content_to_site_settings_table.php]]
- [[_COMMUNITY_ScheduleConflictService|ScheduleConflictService]]
- [[_COMMUNITY_EcommerceOrderItem.php|EcommerceOrderItem.php]]
- [[_COMMUNITY_EcommerceOrderStatusHistory.php|EcommerceOrderStatusHistory.php]]
- [[_COMMUNITY_GuardIntendedUrlIsolationTest|GuardIntendedUrlIsolationTest]]
- [[_COMMUNITY_.managerView|.managerView]]
- [[_COMMUNITY_cms.js|cms.js]]
- [[_COMMUNITY_blogcreate.blade.php|blog/create.blade.php]]
- [[_COMMUNITY_CategoryTranslation.php|CategoryTranslation.php]]
- [[_COMMUNITY_MediaTranslation.php|MediaTranslation.php]]
- [[_COMMUNITY_productsedit.blade.php|products/edit.blade.php]]
- [[_COMMUNITY_productspartials_form.blade.php|products/partials/_form.blade.php]]
- [[_COMMUNITY_projectscreate.blade.php|projects/create.blade.php]]
- [[_COMMUNITY_projectsedit.blade.php|projects/edit.blade.php]]
- [[_COMMUNITY_SiteHomepageSectionTranslation.php|SiteHomepageSectionTranslation.php]]
- [[_COMMUNITY_0000_12_31_235950_create_media_table.php|0000_12_31_235950_create_media_table.php]]
- [[_COMMUNITY_0001_01_01_000002_create_jobs_table.php|0001_01_01_000002_create_jobs_table.php]]
- [[_COMMUNITY_0001_01_01_000003_create_roles_table.php|0001_01_01_000003_create_roles_table.php]]
- [[_COMMUNITY_0001_01_01_000005_create_permission_role_table.php|0001_01_01_000005_create_permission_role_table.php]]
- [[_COMMUNITY_DateTimeHelper|DateTimeHelper]]
- [[_COMMUNITY_slidersedit.blade.php|sliders/edit.blade.php]]
- [[_COMMUNITY_sliderspartials_form.blade.php|sliders/partials/_form.blade.php]]
- [[_COMMUNITY_admin.layouts.main.sidebar._sidebar_item|admin.layouts.main.sidebar._sidebar_item]]
- [[_COMMUNITY_2025_12_14_192415_create_blog_posts_table.php|2025_12_14_192415_create_blog_posts_table.php]]
- [[_COMMUNITY_SiteSettingTranslation.php|SiteSettingTranslation.php]]
- [[_COMMUNITY_admin.pages.ecommerce.orders.partials._item-row|admin.pages.ecommerce.orders.partials._item-row]]
- [[_COMMUNITY_admin.pages.members.partials._form|admin.pages.members.partials._form]]
- [[_COMMUNITY_admin.pages.products.index|admin.pages.products.index]]
- [[_COMMUNITY_admin.pages.projects.index|admin.pages.projects.index]]
- [[_COMMUNITY_admin.partials.topbar-user-dropdown|admin.partials.topbar-user-dropdown]]
- [[_COMMUNITY_create.js|create.js]]
- [[_COMMUNITY_library-attach.js|library-attach.js]]
- [[_COMMUNITY_lock.js|lock.js]]
- [[_COMMUNITY_blogpartials_gallery.blade.php|blog/partials/_gallery.blade.php]]
- [[_COMMUNITY_2026_03_27_202618_create_members_table.php|2026_03_27_202618_create_members_table.php]]
- [[_COMMUNITY_categoriesedit.blade.php|categories/edit.blade.php]]
- [[_COMMUNITY_edit.js|edit.js]]
- [[_COMMUNITY_2025_12_19_154954_create_mediables_table.php|2025_12_19_154954_create_mediables_table.php]]
- [[_COMMUNITY_2026_03_27_202737_create_appointment_slots_table.php|2026_03_27_202737_create_appointment_slots_table.php]]
- [[_COMMUNITY_EcommerceOrderItem.php|EcommerceOrderItem.php]]
- [[_COMMUNITY_2026_01_03_120127_create_projects_table.php|2026_01_03_120127_create_projects_table.php]]
- [[_COMMUNITY_messagesshow.blade.php|messages/show.blade.php]]
- [[_COMMUNITY_AdminDashboardPreference.php|AdminDashboardPreference.php]]
- [[_COMMUNITY_2026_02_15_000002_create_category_product_table.php|2026_02_15_000002_create_category_product_table.php]]
- [[_COMMUNITY_2026_03_27_202646_create_appointments_table.php|2026_03_27_202646_create_appointments_table.php]]
- [[_COMMUNITY_0001_01_01_000000_create_users_table.php|0001_01_01_000000_create_users_table.php]]
- [[_COMMUNITY_2026_04_28_000005_create_payment_integrations_table.php|2026_04_28_000005_create_payment_integrations_table.php]]
- [[_COMMUNITY_2026_03_28_120222_create_provider_working_hours_table.php|2026_03_28_120222_create_provider_working_hours_table.php]]
- [[_COMMUNITY_2026_03_28_120252_create_provider_time_offs_table.php|2026_03_28_120252_create_provider_time_offs_table.php]]
- [[_COMMUNITY_2026_03_28_120317_create_global_blackouts_table.php|2026_03_28_120317_create_global_blackouts_table.php]]
- [[_COMMUNITY_up|up]]
- [[_COMMUNITY_2026_04_21_000001_create_contact_messages_table.php|2026_04_21_000001_create_contact_messages_table.php]]
- [[_COMMUNITY_2026_04_22_000005_create_site_pages_table.php|2026_04_22_000005_create_site_pages_table.php]]
- [[_COMMUNITY_2026_04_22_000006_create_site_faqs_table.php|2026_04_22_000006_create_site_faqs_table.php]]
- [[_COMMUNITY_2026_04_22_000008_create_site_navigation_items_table.php|2026_04_22_000008_create_site_navigation_items_table.php]]
- [[_COMMUNITY_2026_04_22_000009_create_site_settings_table.php|2026_04_22_000009_create_site_settings_table.php]]
- [[_COMMUNITY_up|up]]
- [[_COMMUNITY_2026_04_23_000003_create_admin_content_translation_tables.php|2026_04_23_000003_create_admin_content_translation_tables.php]]
- [[_COMMUNITY_2026_04_23_000004_create_admin_dashboard_preferences_table.php|2026_04_23_000004_create_admin_dashboard_preferences_table.php]]
- [[_COMMUNITY_2026_04_29_000001_add_mail_notification_settings_to_site_settings_table.php|2026_04_29_000001_add_mail_notification_settings_to_site_settings_table.php]]
- [[_COMMUNITY_2026_04_30_000002_add_seo_file_settings_to_site_settings_table.php|2026_04_30_000002_add_seo_file_settings_to_site_settings_table.php]]
- [[_COMMUNITY_2026_05_04_000001_create_ecommerce_order_tables.php|2026_05_04_000001_create_ecommerce_order_tables.php]]
- [[_COMMUNITY_admin_menumodulesnotifications.php|admin_menu/modules/notifications.php]]
- [[_COMMUNITY_admin_menumodulessite_cms.php|admin_menu/modules/site_cms.php]]
- [[_COMMUNITY_admin-module-generator.php|admin-module-generator.php]]
- [[_COMMUNITY_configapp.php|config/app.php]]
- [[_COMMUNITY_member-portal.js|member-portal.js]]
- [[_COMMUNITY_database.php|database.php]]
- [[_COMMUNITY_filesystems.php|filesystems.php]]
- [[_COMMUNITY_logging.php|logging.php]]
- [[_COMMUNITY_membership_terms.php|membership_terms.php]]
- [[_COMMUNITY_payment_integrations.php|payment_integrations.php]]
- [[_COMMUNITY_queue.php|queue.php]]
- [[_COMMUNITY_services.php|services.php]]
- [[_COMMUNITY_session.php|session.php]]
- [[_COMMUNITY_site_ui_labels.php|site_ui_labels.php]]
- [[_COMMUNITY_permissionsmodulesappointments.php|permissions/modules/appointments.php]]
- [[_COMMUNITY_audit_logs.php|audit_logs.php]]
- [[_COMMUNITY_blog.php|blog.php]]
- [[_COMMUNITY_ecommerce_operations.php|ecommerce_operations.php]]
- [[_COMMUNITY_ecommerce_orders.php|ecommerce_orders.php]]
- [[_COMMUNITY_galleries.php|galleries.php]]
- [[_COMMUNITY_admin.pages.ecommerce.orders.partials._item-row|admin.pages.ecommerce.orders.partials._item-row]]
- [[_COMMUNITY_permissionsmodulesmembers.php|permissions/modules/members.php]]
- [[_COMMUNITY_permissionsmodulesmessages.php|permissions/modules/messages.php]]
- [[_COMMUNITY_permissionsmodulesnotifications.php|permissions/modules/notifications.php]]
- [[_COMMUNITY_products.php|products.php]]
- [[_COMMUNITY_permissionsmodulessite_cms.php|permissions/modules/site_cms.php]]
- [[_COMMUNITY_site_payments.php|site_payments.php]]
- [[_COMMUNITY_2026_05_06_000003_create_ecommerce_operation_tables.php|2026_05_06_000003_create_ecommerce_operation_tables.php]]
- [[_COMMUNITY_2026_08_01_000001_create_service_review_tables.php|2026_08_01_000001_create_service_review_tables.php]]
- [[_COMMUNITY_2026_08_04_000001_add_site_portal_workflow.php|2026_08_04_000001_add_site_portal_workflow.php]]
- [[_COMMUNITY_up|up]]
- [[_COMMUNITY_2026_08_18_000004_add_palette_to_site_settings.php|2026_08_18_000004_add_palette_to_site_settings.php]]
- [[_COMMUNITY_footer.blade.php|footer.blade.php]]
- [[_COMMUNITY_head.blade.php|head.blade.php]]
- [[_COMMUNITY_calendar.blade.php|calendar.blade.php]]
- [[_COMMUNITY_settings.blade.php|settings.blade.php]]
- [[_COMMUNITY_audit-logsindex.blade.php|audit-logs/index.blade.php]]
- [[_COMMUNITY_audit-logsshow.blade.php|audit-logs/show.blade.php]]
- [[_COMMUNITY_forgot-password.blade.php|forgot-password.blade.php]]
- [[_COMMUNITY_authindex.blade.php|auth/index.blade.php]]
- [[_COMMUNITY_reset-password.blade.php|reset-password.blade.php]]
- [[_COMMUNITY_blogindex.blade.php|blog/index.blade.php]]
- [[_COMMUNITY_categoriesindex.blade.php|categories/index.blade.php]]
- [[_COMMUNITY__actions.blade.php|_actions.blade.php]]
- [[_COMMUNITY__trash_actions.blade.php|_trash_actions.blade.php]]
- [[_COMMUNITY_partialstrash.blade.php|partials/trash.blade.php]]
- [[_COMMUNITY_dashindex.blade.php|dash/index.blade.php]]
- [[_COMMUNITY_manage.blade.php|manage.blade.php]]
- [[_COMMUNITY_couponsform.blade.php|coupons/form.blade.php]]
- [[_COMMUNITY_couponsindex.blade.php|coupons/index.blade.php]]
- [[_COMMUNITY_inventoryindex.blade.php|inventory/index.blade.php]]
- [[_COMMUNITY_invoicesindex.blade.php|invoices/index.blade.php]]
- [[_COMMUNITY_ordersindex.blade.php|orders/index.blade.php]]
- [[_COMMUNITY__item-row.blade.php|_item-row.blade.php]]
- [[_COMMUNITY_ordersshow.blade.php|orders/show.blade.php]]
- [[_COMMUNITY_webhooksindex.blade.php|webhooks/index.blade.php]]
- [[_COMMUNITY_galleriescreate.blade.php|galleries/create.blade.php]]
- [[_COMMUNITY_galleriesindex.blade.php|galleries/index.blade.php]]
- [[_COMMUNITY__upload-modal.blade.php|_upload-modal.blade.php]]
- [[_COMMUNITY_membersindex.blade.php|members/index.blade.php]]
- [[_COMMUNITY_memberspartials_form.blade.php|members/partials/_form.blade.php]]
- [[_COMMUNITY_menu-visibilityedit.blade.php|menu-visibility/edit.blade.php]]
- [[_COMMUNITY_notificationsindex.blade.php|notifications/index.blade.php]]
- [[_COMMUNITY_productsindex.blade.php|products/index.blade.php]]
- [[_COMMUNITY_profileedit.blade.php|profile/edit.blade.php]]
- [[_COMMUNITY_profileindex.blade.php|profile/index.blade.php]]
- [[_COMMUNITY_projectsindex.blade.php|projects/index.blade.php]]
- [[_COMMUNITY_languagesindex.blade.php|languages/index.blade.php]]
- [[_COMMUNITY_pagesindex.blade.php|pages/index.blade.php]]
- [[_COMMUNITY_paymentsindex.blade.php|payments/index.blade.php]]
- [[_COMMUNITY_paymentspartials_form.blade.php|payments/partials/_form.blade.php]]
- [[_COMMUNITY_slidersindex.blade.php|sliders/index.blade.php]]
- [[_COMMUNITY_trashindex.blade.php|trash/index.blade.php]]
- [[_COMMUNITY__flash.blade.php|_flash.blade.php]]
- [[_COMMUNITY_logo.blade.php|logo.blade.php]]
- [[_COMMUNITY__picker-modal.blade.php|_picker-modal.blade.php]]
- [[_COMMUNITY_up|up]]
- [[_COMMUNITY_index.js|index.js]]
- [[_COMMUNITY_Seeding notes (safe defaults)|Seeding notes (safe defaults)]]
- [[_COMMUNITY_showAlertMessage|showAlertMessage]]
- [[_COMMUNITY_home-hero-computer.blade.php|home-hero-computer.blade.php]]
- [[_COMMUNITY_2026_05_06_000001_create_admin_notifications_table.php|2026_05_06_000001_create_admin_notifications_table.php]]
- [[_COMMUNITY_.categories|.categories]]

## God Nodes (most connected - your core abstractions)
1. `User` - 179 edges
2. `Controller` - 124 edges
3. `Project` - 92 edges
4. `Product` - 73 edges
5. `Appointment` - 72 edges
6. `AuditEvent` - 66 edges
7. `BlogPost` - 65 edges
8. `EcommerceOrder` - 63 edges
9. `Category` - 60 edges
10. `Member` - 60 edges

## Surprising Connections (you probably didn't know these)
- `AppointmentCalendarController` --inherits--> `Controller`  [EXTRACTED]
  app/Http/Controllers/Admin/Appointment/AppointmentCalendarController.php → app/Http/Controllers/Controller.php
- `AppointmentCalendarController` --references--> `AppointmentService`  [EXTRACTED]
  app/Http/Controllers/Admin/Appointment/AppointmentCalendarController.php → app/Services/Appointment/AppointmentService.php
- `AppointmentSettingsController` --inherits--> `Controller`  [EXTRACTED]
  app/Http/Controllers/Admin/Appointment/AppointmentSettingsController.php → app/Http/Controllers/Controller.php
- `AuditLogController` --inherits--> `Controller`  [EXTRACTED]
  app/Http/Controllers/Admin/AuditLog/AuditLogController.php → app/Http/Controllers/Controller.php
- `AuthController` --inherits--> `Controller`  [EXTRACTED]
  app/Http/Controllers/Admin/Auth/AuthController.php → app/Http/Controllers/Controller.php

## Import Cycles
- None detected.

## Communities (430 total, 154 thin omitted)

### Community 0 - "Illuminate\Http\RedirectResponse"
Cohesion: 0.06
Nodes (17): AppointmentSettingsController, Carbon, Collection, JsonResponse, Request, AppointmentController, Request, AppointmentMeetingMethod (+9 more)

### Community 1 - "BlogPost"
Cohesion: 0.06
Nodes (17): BlogPostController, JsonResponse, RedirectResponse, Request, View, BlogPostGalleryController, JsonResponse, Request (+9 more)

### Community 2 - "history-timeline.js"
Cohesion: 0.06
Nodes (74): addDays(), addMonths(), createTimelineToolbar(), dateKey(), dateParts(), defaultAnchorForView(), destroyHistoryTimeline(), destroyHistoryTimelines() (+66 more)

### Community 3 - "calendar.js"
Cohesion: 0.13
Nodes (8): TrashPurgeCommand, CategoryController, JsonResponse, Request, View, Category, BelongsTo, HasMany

### Community 4 - "AuditEvent"
Cohesion: 0.04
Nodes (43): down(), up(), down(), up(), down(), up(), down(), up() (+35 more)

### Community 5 - "EcommerceOrder"
Cohesion: 0.07
Nodes (69): appointmentModalState, blockModalState, buildEventContent(), buildEventsUrl(), buildEventTooltipHtml(), calcBlocks(), closeAppointmentModal(), closeBlockModal() (+61 more)

### Community 6 - "Role"
Cohesion: 0.23
Nodes (4): OrderController, Carbon, RedirectResponse, Request

### Community 7 - "Project"
Cohesion: 0.06
Nodes (11): AdminModuleInstallCommand, MakeAdminModule, SyncServiceReviews, AdminModuleInstaller, FilePatcher, ModuleGenerator, Command, ModuleNamer (+3 more)

### Community 8 - "ModuleNamer"
Cohesion: 0.07
Nodes (23): AdminMiddleware, Closure, Request, AjaxRedirectResponseMiddleware, Closure, Request, Response, DemoAccessMiddleware (+15 more)

### Community 10 - "PaymentIntegration"
Cohesion: 0.08
Nodes (7): PaymentIntegrationController, RedirectResponse, Request, View, PaymentIntegration, Builder, PaymentProviderRegistry

### Community 11 - "dependencies"
Cohesion: 0.06
Nodes (29): init(), init(), closeModal(), init(), openModal(), init(), init(), NoopPage() (+21 more)

### Community 12 - "Member"
Cohesion: 0.10
Nodes (46): BelongsTo, SiteHomepageSectionItemTranslation, applyScheduleTemplate(), applyTemplateValue(), applyWorkingHourRowState(), BLOCK_META, blockMeta(), collectDays() (+38 more)

### Community 13 - ".now"
Cohesion: 0.04
Nodes (47): pestphp/pest-plugin, php-http/discovery, autoload, autoload-dev, psr-4, psr-4, config, allow-plugins (+39 more)

### Community 14 - "admin/pages/index.js"
Cohesion: 0.10
Nodes (7): DashController, Carbon, Collection, RedirectResponse, Request, View, DashboardSectionRegistry

### Community 15 - "Product"
Cohesion: 0.07
Nodes (7): BelongsTo, Builder, HasMany, MorphOne, MorphToMany, Project, MemberProjectWorkflowService

### Community 16 - "AdminMenuRegistry"
Cohesion: 0.08
Nodes (9): BelongsTo, Builder, Collection, HasMany, SiteNavigationItem, NavigationTree, Collection, SiteNavigationRoutes (+1 more)

### Community 17 - "settings.js"
Cohesion: 0.08
Nodes (5): EcommerceOrder, BelongsTo, Builder, HasMany, MorphOne

### Community 18 - "LocalizedContentTranslationService"
Cohesion: 0.16
Nodes (4): DemoDataFactoryService, Closure, Collection, Model

### Community 19 - ".index"
Cohesion: 0.10
Nodes (5): Builder, HasMany, HasOne, self, User

### Community 20 - "blog/form-shared.js"
Cohesion: 0.14
Nodes (6): JsonResponse, RedirectResponse, Request, View, ProductController, Product

### Community 21 - "ContactMessage"
Cohesion: 0.17
Nodes (4): Galleryable, Model, Notifiable, SoftDeletes

### Community 22 - "Closure"
Cohesion: 0.18
Nodes (5): RedirectResponse, Request, View, SiteSettingsController, SiteMailConfigurator

### Community 23 - "Appointment"
Cohesion: 0.10
Nodes (4): Member, Builder, HasMany, Authenticatable

### Community 24 - "products/form-shared.js"
Cohesion: 0.11
Nodes (13): HomepageSectionController, JsonResponse, RedirectResponse, Request, View, Builder, HasMany, SiteHomepageSection (+5 more)

### Community 25 - "projects/form-shared.js"
Cohesion: 0.24
Nodes (18): activateMode(), bindColorField(), bindModeLabels(), bindRangeField(), COMPUTER_COLOR_SETTINGS, init(), numericValue(), safeBlend() (+10 more)

### Community 26 - "AppointmentCalendarController"
Cohesion: 0.12
Nodes (20): StoreContactMessageRequest, init(), clearErrors(), disableSubmitter(), effectiveMethod(), fieldNameCandidates(), fieldWrapper(), firstValidationMessage() (+12 more)

### Community 27 - "EcommerceInvoice"
Cohesion: 0.07
Nodes (28): allowScripts, core-js-pure, dependencies, alpinejs, @fontsource-variable/besley, @fontsource-variable/schibsted-grotesk, @fullcalendar/core, @fullcalendar/daygrid (+20 more)

### Community 28 - "appointments/index.js"
Cohesion: 0.05
Nodes (25): HomeSliderController, JsonResponse, RedirectResponse, Request, View, NavigationController, JsonResponse, RedirectResponse (+17 more)

### Community 29 - "ServiceReviewQuestion"
Cohesion: 0.17
Nodes (17): createImagePopover(), createProductFilter(), createStatusPopover(), hideImagePopover(), hideStatusPopover(), init(), notify(), parseStatusOptions() (+9 more)

### Community 30 - "Illuminate\Bus\Queueable"
Cohesion: 0.07
Nodes (8): ContactMessageController, JsonResponse, RedirectResponse, Request, View, ContactMessage, BelongsTo, Builder

### Community 31 - "Category"
Cohesion: 0.17
Nodes (8): GalleryController, JsonResponse, Request, Gallery, Builder, HasMany, HasOne, AuditEvent

### Community 32 - "AdminQuickSearchController"
Cohesion: 0.20
Nodes (4): JsonResponse, RedirectResponse, Request, ProjectController

### Community 33 - "User"
Cohesion: 0.20
Nodes (24): init(), init(), notify(), clampTextLength(), countWords(), csrfToken(), debounce(), formatPrice() (+16 more)

### Community 34 - "0001_01_01_000001_create_cache_table.php"
Cohesion: 0.20
Nodes (24): init(), init(), notify(), clampTextLength(), countWords(), csrfToken(), debounce(), getTheme() (+16 more)

### Community 35 - "HomepageConfigurationService"
Cohesion: 0.14
Nodes (8): AdminMenuVisibilityController, RedirectResponse, Request, View, AdminMenuSetting, BelongsTo, AdminMenuRegistry, AdminMenuVisibility

### Community 36 - "SeoFileGenerator"
Cohesion: 0.10
Nodes (7): StoreCategoryRequest, UpdateCategoryRequest, CategoryTranslation, BelongsTo, CategoryTree, Collection, FormRequest

### Community 37 - "projects/index.js"
Cohesion: 0.15
Nodes (13): SendAppointmentAdminNotificationMailJob, SendAppointmentUpdatedMailJob, SendContactMessageReceivedMailJob, AdminAppointmentNotificationMail, AdminContactMessageReceivedMail, AppointmentUpdatedMail, SiteMailTestMail, Dispatchable (+5 more)

### Community 38 - "Media"
Cohesion: 0.11
Nodes (9): Appointment, BelongsTo, HasMany, HasOne, MorphOne, self, AppointmentService, Carbon (+1 more)

### Community 39 - "HomeSlider"
Cohesion: 0.23
Nodes (5): AppointmentObserver, EcommerceOrderObserver, AppServiceProvider, ServiceProvider, ShouldHandleEventsAfterCommit

### Community 40 - "Illuminate\Database\Eloquent\Builder"
Cohesion: 0.22
Nodes (5): AppointmentSlot, BelongsTo, BelongsTo, ProviderWorkingHour, HasFactory

### Community 41 - "products/index.js"
Cohesion: 0.08
Nodes (24): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+16 more)

### Community 42 - "ProfileController"
Cohesion: 0.21
Nodes (22): init(), init(), notify(), clampTextLength(), countWords(), csrfToken(), debounce(), getTheme() (+14 more)

### Community 43 - "SitePage"
Cohesion: 0.16
Nodes (27): bindActiveAppointmentActions(), bindStepperControls(), calendarSkeletonCells(), cancelAppointment(), confirmBooking(), confirmReschedule(), createSlotElement(), currentMonth (+19 more)

### Community 44 - "Illuminate\Foundation\Http\FormRequest"
Cohesion: 0.10
Nodes (10): ContentPageController, Carbon, JsonResponse, RedirectResponse, Request, View, BelongsTo, Builder (+2 more)

### Community 45 - "AdminNotification"
Cohesion: 0.21
Nodes (6): InvoiceController, RedirectResponse, Request, View, EcommerceInvoice, BelongsTo

### Community 46 - "ajax-forms.js"
Cohesion: 0.08
Nodes (23): Adding Your Own Content, Architecture, Available Routes, Build for Production, ✅ Core Implementation, Customization, Demo Layouts, Design Principles (+15 more)

### Community 47 - "SiteSetting"
Cohesion: 0.13
Nodes (5): BelongsTo, HasMany, SiteSetting, Collection, SeoFileGenerator

### Community 48 - "up"
Cohesion: 0.17
Nodes (4): Builder, HasMany, ServiceReviewQuestion, ServiceReviewFlowTest

### Community 49 - "TestCase"
Cohesion: 0.17
Nodes (19): createImagePopover(), createProjectFilter(), createStatusPopover(), hideImagePopover(), hideStatusPopover(), init(), notify(), parsePublicStatuses() (+11 more)

### Community 50 - "EcommerceCoupon"
Cohesion: 0.08
Nodes (21): InventoryController, JsonResponse, RedirectResponse, Request, View, PaymentWebhookEventController, RedirectResponse, Request (+13 more)

### Community 51 - "AppointmentSettingsController"
Cohesion: 0.12
Nodes (6): AppointmentCalendarController, Request, BelongsTo, ProviderTimeOff, Carbon, ScheduleConflictService

### Community 52 - "MediaService"
Cohesion: 0.29
Nodes (5): Builder, RedirectResponse, Request, View, ServiceReviewController

### Community 53 - "trash/index.js"
Cohesion: 0.16
Nodes (5): BelongsTo, Builder, HasMany, ServiceReview, MorphTo

### Community 55 - "What You Must Do When Invoked"
Cohesion: 0.23
Nodes (7): MemberProjectController, RedirectResponse, Request, StreamedResponse, View, BelongsTo, ProjectFile

### Community 56 - "SiteSettingsController.php"
Cohesion: 0.21
Nodes (5): MemberController, RedirectResponse, Request, View, initLibraryAttach()

### Community 57 - "blog/index.js"
Cohesion: 0.12
Nodes (13): FaqController, JsonResponse, RedirectResponse, Request, View, FaqController, Request, View (+5 more)

### Community 58 - "create-form-accordion.js"
Cohesion: 0.47
Nodes (8): announce(), createLayoutRow(), directChildren(), init(), initLayoutBuilder(), moveItem(), qs(), qsa()

### Community 59 - "scripts"
Cohesion: 0.50
Nodes (7): dashboardSelects(), initDashboardKtSelects(), initScope(), pendingSyncs, prepareSelect(), selectConstructor(), syncSelect()

### Community 60 - "orders/form.js"
Cohesion: 0.29
Nodes (4): AuthController, Request, GuardIntendedUrl, Request

### Community 61 - "member-register.js"
Cohesion: 0.11
Nodes (17): Commands you will need, Current state, Done criteria, Git workflow, Maintenance notes, Plan 001: Site tasarım temelini kur — fontlar, OKLCH token katmanı, Tailwind @theme eşlemesi, motion altyapısı, Scope, Status (+9 more)

### Community 62 - "http.js"
Cohesion: 0.11
Nodes (17): Commands you will need, Current state, Done criteria, Git workflow, Maintenance notes, Plan 003: Ana sayfayı yeniden tasarla — admin verisi sızıntısını kapat, editoryal bölüm ritmi, crossfade slider, scroll-reveal, Scope, Status (+9 more)

### Community 63 - "SiteDefaultLocalePromotionService"
Cohesion: 0.11
Nodes (17): Commands you will need, Current state, Done criteria, Git workflow, Maintenance notes, Ortak dönüşüm kuralları (her sayfaya uygula), Plan 005: İkincil sayfaları sisteme hizala — auth, iletişim, CMS sayfası, hesap; SEO skoru sızıntısını kaldır, Scope (+9 more)

### Community 64 - "Theme Tailwind HTML Laravel Integration"
Cohesion: 0.25
Nodes (3): AdminQuickSearchController, JsonResponse, Request

### Community 65 - "featured-image-manager.js"
Cohesion: 0.22
Nodes (5): MediaController, JsonResponse, Request, Media, HasMany

### Community 66 - "SiteFaq"
Cohesion: 0.20
Nodes (3): Request, ProfileController, AuditWriter

### Community 67 - "AdminResetPasswordNotification.php"
Cohesion: 0.12
Nodes (16): Commands you will need, Current state, Done criteria, Git workflow, Maintenance notes, Plan 002: Site header, navigasyon ve footer'ı yeniden tasarla — mobil menü, erişilebilir dropdown'lar, editoryal footer, Scope, Status (+8 more)

### Community 68 - "admin/app.js"
Cohesion: 0.12
Nodes (16): Commands you will need, Current state, Done criteria, Git workflow, Maintenance notes, Plan 004: Randevu deneyimini yeniden tasarla — adımlı akış hissi, takvim/slot durum stilleri, hafta günü başlıkları, Scope, Status (+8 more)

### Community 69 - "composer.json"
Cohesion: 0.21
Nodes (6): JsonResponse, Request, TinyMceController, MediaService, UploadedFile, ImageManager

### Community 70 - "date-input.js"
Cohesion: 0.12
Nodes (10): AdminNotificationController, JsonResponse, RedirectResponse, Request, View, AdminNotification, BelongsTo, Builder (+2 more)

### Community 71 - "datatable-helper.js"
Cohesion: 0.29
Nodes (5): GalleryItemsController, JsonResponse, Request, GalleryItem, BelongsTo

### Community 72 - "categories/index.js"
Cohesion: 0.27
Nodes (15): bound, buildDialogButtonClass(), buildErrorMessage(), cloneTpl(), esc(), init(), jsonReq(), markBound() (+7 more)

### Community 73 - "gallery-manager.js"
Cohesion: 0.20
Nodes (6): CouponController, RedirectResponse, Request, View, EcommerceCoupon, Builder

### Community 74 - "swal-alert.js"
Cohesion: 0.16
Nodes (4): self, UserFactory, Factory, static

### Community 75 - "admin/layouts/main/app.blade.php"
Cohesion: 0.10
Nodes (8): HomeController, View, localizedValue(), Model, translationFor(), Collection, Request, SiteLocalization

### Community 77 - "Illuminate\Http\Request"
Cohesion: 0.19
Nodes (11): AppInit(), createCtx(), getPageName(), getPageRoot(), initKtUi(), enhance, _ranFor, _registry (+3 more)

### Community 79 - "settings/edit.js"
Cohesion: 0.36
Nodes (4): DemoDataFactoryController, RedirectResponse, Request, View

### Community 80 - "admin-quick-search.js"
Cohesion: 0.50
Nodes (8): buildSummary(), cardHint(), cardTitle(), cleanText(), init(), openIfNeeded(), wrapLocalizedSections(), wrapTopLevelCards()

### Community 81 - "metronic-pickers.js"
Cohesion: 0.35
Nodes (14): activeOperations, blockForm(), BLOCKING_METHODS, effectiveFormMethod(), ensureOverlay(), hideBlockUi(), initGlobalBlockUi(), installFetchInterceptor() (+6 more)

### Community 82 - "Carbon\Carbon"
Cohesion: 0.29
Nodes (14): buildSummary(), collectDirectCards(), collectSectionCards(), findAutoPages(), hasInteractiveHeader(), initCreateFormAccordions(), normalizeText(), sectionHeader() (+6 more)

### Community 83 - "StoreContactMessageRequest"
Cohesion: 0.35
Nodes (13): addItemRow(), calculateRow(), findPageRoot(), hydrateProduct(), initEcommerceOrderForm(), initKtSelects(), initMemberAutofill(), money() (+5 more)

### Community 84 - "Plan 003: Ana sayfayı yeniden tasarla — admin verisi sızıntısını kapat, editoryal bölüm ritmi, crossfade slider, scroll-reveal"
Cohesion: 0.27
Nodes (12): csrfToken(), destroy(), get(), handleGlobalAuthExpiry(), HttpError, notify(), parseResponseBody(), post() (+4 more)

### Community 87 - "media-upload-modal.js"
Cohesion: 0.16
Nodes (5): StreamedResponse, ProjectStoreRequest, ProjectUpdateRequest, BelongsTo, ProjectTranslation

### Community 88 - "OrderController"
Cohesion: 0.40
Nodes (4): RedirectResponse, Request, View, ServiceReviewQuestionController

### Community 89 - "0001_01_01_000005_create_permission_role_table.php"
Cohesion: 0.22
Nodes (4): BelongsTo, SitePageTranslation, Model, SiteDefaultLocalePromotionService

### Community 90 - "ContentPageController"
Cohesion: 0.35
Nodes (4): RedirectResponse, Request, View, ServiceReviewController

### Community 91 - "Plan 005: İkincil sayfaları sisteme hizala — auth, iletişim, CMS sayfası, hesap; SEO skoru sızıntısını kaldır"
Cohesion: 0.12
Nodes (8): PermissionController, Request, Permission, AdminRoleProfile, Rbac, AdminSeeder, PermissionSeeder, SuperAdminSeeder

### Community 93 - "manage.js"
Cohesion: 0.45
Nodes (11): cleanupObjectUrl(), emitFeaturedChange(), ensureGlobalListeners(), ensureHostInited(), getHost(), handleClearClick(), handleFileChange(), handleMediaPick() (+3 more)

### Community 94 - "initSliderForm"
Cohesion: 0.31
Nodes (4): MemberAuthController, RedirectResponse, Request, View

### Community 95 - "MemberDocumentService"
Cohesion: 0.36
Nodes (4): MemberPasswordResetController, RedirectResponse, Request, View

### Community 96 - "galleries/edit.js"
Cohesion: 0.16
Nodes (10): AuditLogController, Builder, RedirectResponse, Request, AuditRequestMiddleware, Closure, Request, Response (+2 more)

### Community 97 - "menu-visibility/edit.js"
Cohesion: 0.23
Nodes (6): loaderFallback, escapeAttr(), escapeHtml(), getPickerEls(), hideModal(), initMediaPicker()

### Community 98 - "psr-4"
Cohesion: 0.32
Nodes (3): JsonResponse, Request, ProductGalleryController

### Community 101 - "blog/partials/_form.blade.php"
Cohesion: 0.29
Nodes (11): getDateInputValue(), inferMode(), initDateInputValues(), pad(), parseMachineDate(), setDateInputValue(), toConfiguredDateValue(), todayMachineDate() (+3 more)

### Community 102 - "projects/partials/_form.blade.php"
Cohesion: 0.40
Nodes (4): PasswordResetController, RedirectResponse, Request, View

### Community 105 - "post-create-project-cmd"
Cohesion: 0.43
Nodes (4): HomepageConfigurationController, RedirectResponse, Request, View

### Community 106 - "2026_08_01_000002_create_site_homepage_configs_table.php"
Cohesion: 0.30
Nodes (13): createBlogFilter(), createPopover(), hideImgPopover(), init(), notify(), postJson(), redrawOwningTable(), renderPagination() (+5 more)

### Community 107 - "ServiceReview"
Cohesion: 0.24
Nodes (4): HtmlSanitizer, DOMDocument, DOMElement, DOMNode

### Community 108 - "slug-manager.js"
Cohesion: 0.38
Nodes (9): applyEmptyState(), initDataTable(), isTrTemplate(), markBound(), normalizeRowTemplate(), renderPagination(), resolveTableEl(), safeDestroyDt() (+1 more)

### Community 109 - "base.blade.php"
Cohesion: 0.31
Nodes (6): escHtml(), init(), qs(), renderActions(), renderConnections(), renderRow()

### Community 110 - "homepage/edit.js"
Cohesion: 0.29
Nodes (6): attachedRowHtml(), escapeHtml(), initGalleryManager(), jreq(), mountOne(), renderPager()

### Community 111 - "Sluggable"
Cohesion: 0.22
Nodes (8): admin.layouts.main.footer, admin.layouts.main.header, admin.layouts.main.sidebar, admin.partials.media._picker-modal, admin.partials.modals.search, admin.layouts.partials.head, admin.layouts.partials.scripts, admin.partials.theme-toggle

### Community 112 - "autoload-dev"
Cohesion: 0.53
Nodes (4): Closure, Request, Response, SiteLocaleMiddleware

### Community 115 - "0001_01_01_000000_create_users_table.php"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 116 - "AdminAppointmentNotificationMail"
Cohesion: 0.20
Nodes (5): AdminResetPasswordNotification, MailMessage, MemberResetPasswordNotification, MailMessage, Notification

### Community 117 - ".schema"
Cohesion: 0.39
Nodes (7): closeModal(), debounce(), escapeHtml(), groupMarkup(), initAdminQuickSearch(), isTypingTarget(), itemMarkup()

### Community 118 - "0001_01_01_000003_create_roles_table.php"
Cohesion: 0.36
Nodes (12): createAction(), decoratePicker(), flatpickr(), initDatePicker(), initMetronicPickers(), initScope(), initTimePicker(), normalizeKtFormat() (+4 more)

### Community 119 - "0001_01_01_000004_create_permissions_table.php"
Cohesion: 0.44
Nodes (8): buildConfirmButtonClass(), buildToastClass(), getMeta(), normalizeText(), resolveSwal(), showConfirmDialog(), showToastMessage(), TYPE_META

### Community 120 - "homepage/edit.blade.php"
Cohesion: 0.22
Nodes (8): checkbox, closeButtons, modal, modalStatus, openButtons, readInput, scrollable, status

### Community 122 - "2025_12_14_192415_create_blog_posts_table.php"
Cohesion: 0.27
Nodes (3): JsonResponse, Request, TrashController

### Community 123 - "2025_12_16_142024_create_categories_table.php"
Cohesion: 0.50
Nodes (3): JsonResponse, Request, ProjectGalleryController

### Community 125 - "site_homepage.php"
Cohesion: 0.43
Nodes (4): MemberAccountController, RedirectResponse, Request, View

### Community 128 - "2025_12_27_133537_create_gallery_items_table.php"
Cohesion: 0.46
Nodes (7): classifyPanel(), classifyWithin(), directTitle(), hasCollectionStructure(), hasDirectForm(), initAdminSemanticPanels(), isTopLevelPanel()

### Community 129 - "2025_12_27_133604_create_galleryables_table.php"
Cohesion: 0.07
Nodes (13): BaseTestCase, AuthSeeder, DatabaseSeeder, HomepageConfigPermissionSeeder, ServiceReviewPermissionSeeder, RefreshDatabase, Seeder, AdminDashboardLayoutTest (+5 more)

### Community 131 - ".show"
Cohesion: 0.20
Nodes (9): Controller, View, ServiceController, GalleryController, Request, View, AuthorizesRequests, BaseController (+1 more)

### Community 132 - "2026_02_15_000002_create_category_product_table.php"
Cohesion: 0.46
Nodes (4): attachGridActions(), attachBulkActions(), init(), createMediaList()

### Community 134 - "SiteLanguage"
Cohesion: 0.36
Nodes (4): btnBusy(), initMediaUploadModal(), preventAll(), setRing()

### Community 135 - "ProjectStoreRequest"
Cohesion: 0.46
Nodes (7): asElement(), describedByIds(), ensureAccessibleName(), initTitleTooltips(), SPECIALIZED_TOOLTIP_SELECTOR, upgradeTitle(), upgradeTitlesWithin()

### Community 136 - "ProductGalleryController"
Cohesion: 0.42
Nodes (5): BlogController, Builder, JsonResponse, Request, View

### Community 137 - "2026_03_28_120222_create_provider_working_hours_table.php"
Cohesion: 0.46
Nodes (5): init(), init(), bindIconChips(), initPageForm(), syncPreview()

### Community 141 - "2026_04_22_000005_create_site_pages_table.php"
Cohesion: 0.38
Nodes (3): ContactMessageController, RedirectResponse, View

### Community 142 - "SiteTranslationSyncService"
Cohesion: 0.29
Nodes (6): Add module permissions, Built-in role profiles, Control demo user creation, Customize credentials, Default behavior, Seeding notes (safe defaults)

### Community 143 - "2026_04_22_000007_create_site_counters_table.php"
Cohesion: 0.29
Nodes (6): Aesthetic Direction, Brand Personality, Design Context, Design Principles, Impeccable — Proje Tasarım Bağlamı, Users

### Community 144 - "2026_04_22_000008_create_site_navigation_items_table.php"
Cohesion: 0.13
Nodes (6): Request, RoleController, Role, AdminLoginBrandingTest, AdminRoleProfileTest, SiteServicesAndConsultationTest

### Community 145 - "2026_04_22_000009_create_site_settings_table.php"
Cohesion: 0.22
Nodes (8): partials.favicons, site.partials.home-hero-computer, site.partials.theme-bootstrap, site.partials.theme-toggle, site.home-sections.about-faq, site.home-sections.features, site.home-sections.probablue-hero, site.partials.home-mode-icon

### Community 146 - "AdminLoginBrandingTest"
Cohesion: 0.52
Nodes (4): init(), init(), initSliderForm(), syncPreview()

### Community 151 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 152 - ".boot"
Cohesion: 0.33
Nodes (5): Bulgular — tabloya girmeyen notlar, Dependency notes, Execution order & status, Findings considered and rejected, Implementation Plans

### Community 153 - "AdminDashboardPreference.php"
Cohesion: 0.12
Nodes (5): PageController, View, HasMany, SiteHomepageConfig, HasSiteLocaleTranslations

### Community 154 - "2026_04_30_000003_add_sitemap_xml_content_to_site_settings_table.php"
Cohesion: 0.67
Nodes (5): effectiveVisibleCount(), init(), normalize(), qsa(), syncCard()

### Community 155 - "ScheduleConflictService"
Cohesion: 0.29
Nodes (6): partials.favicons, site.partials.theme-bootstrap, site.partials.theme-toggle, site.partials.navigation.desktop-item, site.partials.navigation.footer-item, site.partials.navigation.mobile-item

### Community 160 - "cms.js"
Cohesion: 0.70
Nodes (4): findScript(), loadScriptOnce(), normalizedUrl(), scriptPromises

### Community 161 - "blog/create.blade.php"
Cohesion: 0.40
Nodes (4): categoryInput, form, resultsSection, searchInput

### Community 164 - "products/edit.blade.php"
Cohesion: 0.50
Nodes (3): admin.pages.blog.partials._gallery, admin.components.featured-image-manager, admin.components.localized-content-tabs

### Community 165 - "products/partials/_form.blade.php"
Cohesion: 0.50
Nodes (3): admin.pages.projects.partials._gallery, admin.components.featured-image-manager, admin.components.localized-content-tabs

### Community 166 - "projects/create.blade.php"
Cohesion: 0.11
Nodes (14): RedirectResponse, Request, View, SiteLanguageController, HomeSliderTranslation, BelongsTo, BelongsTo, SiteCounterTranslation (+6 more)

### Community 167 - "projects/edit.blade.php"
Cohesion: 1.00
Nodes (3): media(), mediaIn(), MorphToMany

### Community 170 - "0001_01_01_000002_create_jobs_table.php"
Cohesion: 0.83
Nodes (3): init(), serializeTree(), toggleLinkFields()

### Community 174 - "sliders/edit.blade.php"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 175 - "sliders/partials/_form.blade.php"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 176 - "admin.layouts.main.sidebar._sidebar_item"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 179 - "admin.pages.ecommerce.orders.partials._item-row"
Cohesion: 0.67
Nodes (3): initSlugManager(), slugifyTR(), TURKISH_CHAR_MAP

### Community 180 - "admin.pages.members.partials._form"
Cohesion: 0.50
Nodes (3): fileInput, fileSelection, terminationForm

### Community 181 - "admin.pages.products.index"
Cohesion: 0.50
Nodes (3): admin.layouts.partials.head, admin.layouts.partials.scripts, admin.partials.theme-toggle

### Community 185 - "create.js"
Cohesion: 0.39
Nodes (3): init(), init(), initPermissionsForm()

## Knowledge Gaps
- **376 isolated node(s):** `$schema`, `name`, `type`, `description`, `keywords` (+371 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **154 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Controller` connect `.show` to `Illuminate\Http\RedirectResponse`, `BlogPost`, `calendar.js`, `Role`, `ProductGalleryController`, `PaymentIntegration`, `2026_04_22_000005_create_site_pages_table.php`, `admin/pages/index.js`, `2026_04_22_000008_create_site_navigation_items_table.php`, `blog/form-shared.js`, `ContactMessage`, `Closure`, `products/form-shared.js`, `AdminDashboardPreference.php`, `appointments/index.js`, `Illuminate\Bus\Queueable`, `Category`, `AdminQuickSearchController`, `HomepageConfigurationService`, `SeoFileGenerator`, `projects/create.blade.php`, `Illuminate\Foundation\Http\FormRequest`, `AdminNotification`, `EcommerceCoupon`, `AppointmentSettingsController`, `MediaService`, `What You Must Do When Invoked`, `SiteSettingsController.php`, `blog/index.js`, `orders/form.js`, `Theme Tailwind HTML Laravel Integration`, `featured-image-manager.js`, `SiteFaq`, `composer.json`, `date-input.js`, `datatable-helper.js`, `gallery-manager.js`, `admin/layouts/main/app.blade.php`, `settings/edit.js`, `media-upload-modal.js`, `OrderController`, `ContentPageController`, `Plan 005: İkincil sayfaları sisteme hizala — auth, iletişim, CMS sayfası, hesap; SEO skoru sızıntısını kaldır`, `initSliderForm`, `MemberDocumentService`, `galleries/edit.js`, `psr-4`, `require`, `projects/partials/_form.blade.php`, `navigation/index.js`, `post-create-project-cmd`, `2025_12_14_192415_create_blog_posts_table.php`, `2025_12_16_142024_create_categories_table.php`, `site_homepage.php`?**
  _High betweenness centrality (0.110) - this node is a cross-community bridge._
- **Why does `Media` connect `featured-image-manager.js` to `BlogPost`, `calendar.js`, `Illuminate\Database\Eloquent\Model`, `admin/pages/index.js`, `Product`, `2026_04_22_000008_create_site_navigation_items_table.php`, `LocalizedContentTranslationService`, `ContactMessage`, `AdminDashboardPreference.php`, `.managerView`, `SiteSetting`, `Theme Tailwind HTML Laravel Integration`, `SiteFaq`, `composer.json`, `datatable-helper.js`, `swal-alert.js`, `require`, ``appointments``, `PaymentWebhookEvent`, `2025_12_14_192415_create_blog_posts_table.php`, `static`?**
  _High betweenness centrality (0.106) - this node is a cross-community bridge._
- **Why does `User` connect `.index` to `Illuminate\Http\RedirectResponse`, `2025_12_27_133604_create_galleryables_table.php`, `2026_04_22_000005_create_site_pages_table.php`, `admin/pages/index.js`, `2026_04_22_000008_create_site_navigation_items_table.php`, `AdminMenuRegistry`, `LocalizedContentTranslationService`, `ContactMessage`, `Closure`, `Appointment`, `AppointmentCalendarController`, `EcommerceOrderItem.php`, `Illuminate\Bus\Queueable`, `.managerView`, `HomepageConfigurationService`, `Media`, `Illuminate\Database\Eloquent\Builder`, `up`, `AppointmentSettingsController`, `MediaService`, `Theme Tailwind HTML Laravel Integration`, `SiteFaq`, `date-input.js`, `Plan 005: İkincil sayfaları sisteme hizala — auth, iletişim, CMS sayfası, hesap; SEO skoru sızıntısını kaldır`, `galleries/edit.js`, `projects/partials/_form.blade.php`, `navigation/index.js`, ``appointments``, `PaymentWebhookEvent`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Are the 26 inferred relationships involving `User` (e.g. with `.index()` and `.show()`) actually correct?**
  _`User` has 26 INFERRED edges - model-reasoned connections that need verification._
- **Are the 9 inferred relationships involving `Project` (e.g. with `.contentQuality()` and `.index()`) actually correct?**
  _`Project` has 9 INFERRED edges - model-reasoned connections that need verification._
- **Are the 13 inferred relationships involving `Product` (e.g. with `.contentQuality()` and `.index()`) actually correct?**
  _`Product` has 13 INFERRED edges - model-reasoned connections that need verification._
- **What connects `$schema`, `name`, `type` to the rest of the system?**
  _376 weakly-connected nodes found - possible documentation gaps or missing edges._