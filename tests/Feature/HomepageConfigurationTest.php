...
class HomepageConfigurationTest extends TestCase
{
    ...
    public function test_homepage_uses_schema_defaults_and_dynamic_color_variables(): void
    {
        ...
        $this->get('/')
            ...
            ->assertDontSee('Araştırmanızın her aşamasında yanınızdayız.')
            ->assertDontSee('İleri İstatistiksel Analizler')
            ->assertDontSee('Basit, şeffaf ve bilimsel bir süreç.')
            ->assertDontSee('id="birlikte-baslayalim"', false)
            ->assertDontSee('data-surface="tint"', false)
            ->assertDontSee('--home-feature-columns: 3', false)
            ->assertDontSee('href="#neler-sunuyoruz"', false)
            ->assertSee(route('site.services.index').'#hizmetler', false)
            ->assertSee(route('site.services.index').'#nasil-ilerliyoruz', false)
            ->assertSee(route('site.services.index').'#birlikte-baslayalim', false)
            ->assertDontSee(route('member.login'), false)
            ->assertDontSee(route('member.register'), false)
    ...
    public function test_superadmin_can_open_and_update_the_custom_homepage_manager(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertSee('Probablue Logo Paleti')
            ...
            ->assertSee('Probablue Kod Dokulu · Sabit görsel')
            ->assertSee('Probablue Canlı Logo · Renk önizlemesi')
